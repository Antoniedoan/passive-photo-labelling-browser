...
class MainApplication(Tk.Tk):
	...
	def insert_row_data(self,ls,n):
		...
		for data in ls:
			...
			if k == 1:
				db = sqlite3.connect('/Users/doanthanh/Downloads/Labeling-Backend-master/Application/lkydata.db')
				cur = db.cursor()
				query_result = cur.execute("UPDATE attributes SET " + label + " = (?) WHERE file = (?)", (1,self.file_chosen_ls[n].cget("text"),))
				db.commit()

	def run_query(self,query,parameters=()):
		with sqlite3.connect('/Users/doanthanh/Downloads/Labeling-Backend-master/Application/lkydata.db') as db:
			cur = db.cursor()
